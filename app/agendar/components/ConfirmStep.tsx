...
import {
  createAppointment, saveClient, loadSavedClient,
  type ServiceItem, type ProfessionalSimple,
} from "../api";
...
export function ConfirmStep({
  ...
  // Default ON se já havia dados salvos anteriormente
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (saveData) {
        saveClient({ name: clientName, email: clientEmail, phone: clientPhone });
      }
      setCode(result.code ?? String(result.id));
    } catch (e: any) {
    ...
    return (
      <div className="w-full flex flex-col items-center gap-8 py-6 text-center">
        ...
        <div className="bg-[#0c0c0c] border border-white/10 rounded-2xl p-6 w-full max-w-md text-left space-y-4 shadow-lg">
          <SuccessRow icon={<Scissors size={16} />} label={service.name} />
        ...
        <div className="flex flex-col items-center gap-2 mt-4">
          <span className="text-white/30 text-[10px] uppercase tracking-widest font-bold">Seu Código</span>
          <span className="text-[#d4aa7a] text-2xl font-black font-mono bg-[#b8853a]/10 px-6 py-2 rounded-lg border border-[#b8853a]/20">
            {code}
          </span>
        </div>
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      ...
      {/* Card — Agendamento (Fundo Sólido para evitar bordas quebradas) */}
      <div className="rounded-2xl border border-white/10 bg-[#0c0c0c] overflow-hidden shadow-lg">
        ...
        <div className="p-5 space-y-5">
          <ReviewRow
            ...
            label="Serviço"
            value={service.name}
            badge={service.price != null ? `R$ ${service.price}` : undefined}
          />
