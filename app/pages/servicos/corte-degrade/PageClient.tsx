...
/* ─── GALERIA EXCLUSIVA DO SERVIÇO ──────────────────────────────────────── */
const galeriaDegrade = [
  "/cortes/corte15.jpeg",
  "/cortes/corte19.png",
  "/cortes/corte10.jpeg",
  "/cortes/corte11.jpeg",
        ...
        <div className="max-w-5xl mx-auto px-4 md:px-8 flex flex-col items-center">
          ...
          >
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4">
              {galeriaDegrade.map((imagem, index) => (
                <motion.div
                  key={index}
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  transition={{ delay: index * 0.1, duration: 0.5 }}
                  className="relative group overflow-hidden rounded-xl bg-white/5 aspect-[4/5] border border-white/5"
                >
                  <Image 
                    src={imagem} 
                    alt={`Exemplo de Corte Degradê ${index + 1}`} 
                    fill 
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl font-serif font-black text-[#f5f1eb] leading-tight mb-6">
              Corte <span className="text-[#d4aa7a] italic">Degradê.</span>
            </h1>
            
            <p className="text-white/60 text-base md:text-lg leading-relaxed font-medium mb-10 text-center">
              A verdadeira obra de arte da barbearia moderna. Transição perfeita do zero (ou navalha) para o topo, sem marcações. Seja um Mid Fade, Low Fade ou High Fade, na Falcão o seu degradê sai impecável e alinhado ao formato do seu rosto.
            </p>
            
            {/* Preço e Tempo */}
            <div className="flex items-center justify-center gap-6 mb-12 bg-white/[0.02] border border-white/5 py-4 px-8 rounded-2xl">
              <div className="text-3xl font-bold text-[#d4aa7a]">R$ 35</div>
              <div className="h-8 w-[1px] bg-white/10"></div>
              <div className="text-white/40 text-xs uppercase tracking-widest font-bold flex flex-col items-start text-left">
                <span>Duração</span>
                <span className="text-white/70">Aprox. 45 min</span>
              </div>
            </div>

            {/* Botão de Agendar */}
            <a 
              href="/agendar?servico=1" 
